...
/**
Class from https://github.com/opentrace-community/opentrace-ios/blob/master/OpenTrace/Bluetrace/EncounterMessageManager.swift

Used in OpenTrace sources. This implementation includes fix for the bug which prevents tempIDs refresh
*/
class EncounterMessageManager {
    ...
    lazy var functions = Functions.functions(region: FirebaseConfig.functionsRegion)
    ...
    func getTempId(onComplete: @escaping (String?) -> Void) {
        ...
        if advtPayloadExpiry == nil ||  Date() > advtPayloadExpiry! {
            fetchBatchTempIdsFromFirebase { [unowned self] (error: Error?, resp: (tempIds: [[String: Any]], refreshDate: Date)?) in
                ...
                self.removeExpired(tempIDs: &tempIDs)
    ...
    func setAdvtPayloadIntoUserDefaultsv2(_ response: (tempIds: [[String: Any]], refreshDate: Date)) -> Data? {
        ...
        guard let validBm = tempIDs.first?["tempID"] as? String else { return nil }
    ...
    private func removeExpired(tempIDs: inout [[String: Any]]) {
        tempIDs.removeAll {
            guard let expiryTime = $0["expiryTime"] as? TimeInterval else { return true }
